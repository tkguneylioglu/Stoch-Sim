...
class InpatientRequest(DES.Event):
    def execute(self):
        ...
        if not found and not state.inpatient_tripping:
...
class InpatientArrival(DES.Event):
    ...
    def execute(self):
        ...
        state.inpatient_tripping = False
...
class RegenerationPoint(DES.Event):
    def execute(self):
        ...
        if len(state.waiting_queue) == 0 and len(state.inpatient_queue) == 0 and state.occupied_scanners == 0 and state.outpatient_schedule[0] == 23 and not state.inpatient_tripping:
            print("found")
...
DES.insertEvent(RegenerationPoint(8 * 60))
DES.runSimulation()
